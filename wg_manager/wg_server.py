...
class WGServerConfig():
    ...
    def save_to_yaml(self, file_name: str = 'wg_server.yaml'):
        '''
        Сохраняет серверный конфигурационный файл по пути `wg_configs/<file_name>`.
        '''
        self._build_conf_dict()

        with open(self.python_base_path.joinpath('wg_configs', file_name), 'w') as f:
            f.write(yaml.dump(self.config_dict))

    def save_to_wg_config(self, file_name: str = 'wg0.conf'):
        '''
        ...
        with open(self.wg_base_path.joinpath(file_name), 'w+') as f:
            f.write(content+'\n')
